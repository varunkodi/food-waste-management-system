...
<?php
	...
	if(date($row['pickup_time'])<date("H:i:s") && $row['expiry_time']<date("H:i:s")){
		$q1="UPDATE request SET status='expired' where status='pending' AND `r_id`='".$row['r_id']."' ";
						...
						<div class="box-news-1">
							...
							<?php if($row['v_id']=='0'){ ?>
								...
								<div class="meta">
									...
									<h4>Expiry Time:  <?php echo $row['expiry_time'];?></h4>
...
<!-- 									
									...
									<h4>City:  <?php echo $row['v_city'];?></h4>
									<!-- <h4>Cooked Time:  <?php echo $row['cooked_time'];?></h4> -->
									<!-- <h4>Pickup Time:  <?php echo $row['pickup_time'];?></h4> -->
									<h4>Food Quantity:  <?php echo $row['food_qty'];?></h4>
									<h4>Request Date:  <?php echo $row['r_date'];?></h4>
									<h4>Cooked Time:  <?php echo $row['cooked_time'];?></h4>
									<h4>Pickup Time:  <?php echo $row['pickup_time'];?></h4>
									<h4>Epiry Time:  <?php echo $row['expiry_time'];?></h4>
									<h4>Address:  <?php echo $row['v_address'];?></h4>
									<?php }?>
									</span>
								</div>
							</div>
						...
						<div class="form-group col-md-12" style="color:black; font-weight: bold;">		
							<?php if(!empty($row['status']) && $row['status']=='pending'){ ?>                         
										...
										<?php } else if(!empty($row['status']) && $row['status']=='expired'){?>
